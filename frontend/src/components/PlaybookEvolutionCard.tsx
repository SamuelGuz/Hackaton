...
function PlaybookSide({
  ...
  const headerLabel = isAfter ? "Ahora · vigente" : "Antes · reemplazado";
  ...
  return (
    <div
      className={`relative flex flex-col rounded-xl border p-5 ${cardClass}`}
    ...
    >
      ...
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-100 leading-tight">
            {channelLabel[playbook.recommendedChannel]}
          </p>
      ...
      <div className="mb-3">
        <div className="flex items-baseline gap-1">
          ...
          <span className="ml-2 text-[11px] text-slate-500">tasa de éxito</span>
        </div>
        ...
        <p className="text-[11px] text-slate-500 mt-1 tabular-nums">
          {playbook.timesSucceeded} éxitos / {playbook.timesUsed} ejecuciones
        </p>
      </div>
    ...
    <section className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg mb-4">
        ...
        <div className="text-xs text-emerald-200/80 leading-snug">
          La nueva versión convierte <span className="font-semibold">2.6×</span> mejor que la anterior. El agente sigue midiendo cada ejecución para refinar más.
        </div>
      </div>
